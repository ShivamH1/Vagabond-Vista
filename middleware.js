const Listing = require("./models/listing"); //using listing model
const Review = require("./models/review"); //using review model
const ExpressError = require("./util/ExpressError"); //Custom ExpressError class
const{ listingSchema, reviewSchema } = require("./schema"); //validation for server side for listing and review

module.exports.isLoggedIn = (req,res,next) => {
    if(!req.isAuthenticated()){ //checks if user is logged in
        req.session.redirectUrl = req.originalUrl;
        req.flash("error","you must be logged in to create listing");
        return res.redirect("/login");
    }
    next();
}

module.exports.saveRedirectUrl = (req,res,next) => { //helps in redirecting to page the user wants to access after login
    if(req.session.redirectUrl){
        res.locals.redirectUrl = req.session.redirectUrl;
    }
    next();
};

module.exports.isOwner = async(req,res,next) => {
    let {id} = req.params;
    let listing = await Listing.findById(id);
    if(!listing.owner._id.equals(res.locals.currUser._id)){ //Adding Authorization
        req.flash("error","You don't have permission to edit");
        return res.redirect(`/listings/${id}`); //always return otherwise it will execute below lines
    }
    next();
}

//validation for listingschema(middleware) pass as parameter
module.exports.validateListing = (req,res,next) => {
    let {error} = listingSchema.validate(req.body);
    if(error){
        let errMsg = error.details.map((el) => el.message).join(",");
        throw new ExpressError(400,errMsg);
    }else{
        next();
    }
};

//validation for reviewschema(middleware) pass as parameter
module.exports.validateReview = (req,res,next) => {
    let {error} = reviewSchema.validate(req.body);
    if(error){
        let errMsg = error.details.map((el) => el.message).join(",");
        throw new ExpressError(400,errMsg);
    }else{
        next();
    }
};

module.exports.isReviewAuthor = async(req,res,next) => { //authorization for reviews
    let {id,reviewId} = req.params;
    let review = await Review.findById(reviewId);
    if(!review.author._id.equals(res.locals.currUser._id)){ //Adding Authorization
        req.flash("error","You are not the author");
        return res.redirect(`/listings/${id}`); //always return otherwise it will execute below lines
    }
    next();
}